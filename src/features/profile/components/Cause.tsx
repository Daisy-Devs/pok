...
const Cause: React.FC<CauseProps> = ({ cause, organization,individual=false }) => {
  const Icon = CAUSE_CATEGORIES.find(
    (category) => category.name === cause,
  )?.icon||<HeartHandshakeIcon/> as unknown as FC<LucideProps>;
  return (
    <div className="flex flex-row gap-3">
      ...
      <div className="flex flex-col gap-1">
        <p className="text-base font-semibold text-secondarText">{individual?organization:cause}</p>
