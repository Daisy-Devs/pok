import { DropdownMenu, DropdownMenuGroup, DropdownMenuItem, DropdownMenuTrigger } from '@/src/components/ui/dropdown-menu'
...
const Financials:FC<FinancialsProps> = ({ campaignData, setCampaignData}) => {
  return (
       ...
       <InputGroup id="goal-amount" className="w-2xs h-13 mt-2">
        ...
        <InputGroupAddon align="inline-end">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded-sm bg-white p-2 border border-border">
              <DropdownMenuGroup>
    ...
    <div className='flex max-w-xs mt-5 bg-green-50 rounded-sm text-secondary-dark p-7'>
        ...
        <p className='text-sm font-semibold w-2xs'>Funds will be locked in a
smart contract until the goal is reached.</p>
    </div>
    </div>
  )
}
