...
async function run() {
  try {
    ...
    // 🔥 Drop the unique index on txHash
    await mongoose.connection.collection("campaigns").dropIndex("txHash_1").catch(() => {
      console.log("⚠️ Index not found, skipping");
    });
    console.log("🗑️ txHash_1 index dropped successfully");
